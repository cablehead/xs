...

    {
        ...
        tokio::spawn(async move {
            while let Some(frame) = recver.recv().await {
                // Skip frames that were generated by this handler and prune old entries
                if generated_frames.contains(&frame.id) {
                    generated_frames.retain(|id| id > &frame.id);
                    continue;
                }
...
mod tests {
    ...
    async fn test_handler_stateless_no_self_loop() {
        ...
        // this is the echo of echo.registered
