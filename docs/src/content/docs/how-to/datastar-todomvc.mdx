...
This tutorial targets Datastar v1.0.0-RC.5 and is no longer maintained. The Datastar SDK has since released breaking changes to its SSE protocol and attribute API. The underlying concepts -- event sourcing, server-driven UI via SSE, hypermedia-style reactivity -- are still valid. The code examples are not.
</Aside>
...
<MuxVideo playbackId="KX01ZSZ8CXv5SVfVwMZKJTcuBcUQmo1ReS9U5JjoHm4k" muted={false} />

![Datastar TodoMVC Demo](../../../assets/datastar.gif)

<CardGrid>
  <Card title="Live Demo" icon="rocket">
    **[Try it live](https://datastar-todomvc.cross.stream)** - historical snapshot
    using Datastar v1.0.0-RC.5.
  </Card>
...
Raw events are awkward for presenting a summary of the current state of things. We want to create a projection of the stream. We can use Nushell's `generate` command to aggregate events into state:
