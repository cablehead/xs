...
<Aside type="tip">
  ...
  [xs/examples/datastar-todomvc](https://github.com/cablehead/xs/tree/main/examples/datastar-todomvc).
</Aside>
...
The Datastar input attributes:

- `data-signals-input` - creates a signal named `$input` for this field's value
- `data-bind-input` - binds the field value to the `$input` signal
- `data-on-keydown` - handles Enter key presses to submit the todo
...
1. Parses the JSON body to extract the `input` field (Datastar sends `{"input": "todo text"}`)
2. Trims whitespace and checks if the text is empty
3. If valid, appends a new todo event to the stream
4. Returns status 204 (No Content) on success
