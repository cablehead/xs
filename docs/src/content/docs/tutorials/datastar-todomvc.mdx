...
Now let's make our todo list available over HTTP. Create a basic `serve.nu` that supports `/` and `/updates` endpoints:
...
Visit http://localhost:3001 to see your reactive TodoMVC. The `/` endpoint serves the `index.html` that uses Datastar, while `/updates` is a Server-Sent Events endpoint that runs our generate command and streams HTML fragments to connected browsers.
