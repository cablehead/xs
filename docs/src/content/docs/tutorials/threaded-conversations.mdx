...
thread (.last chat.turn).id | each {.cas}
```
...
"The Eiffel Tower is about 300 meters tall." | .append chat.turn --meta {role: assistant continues: (.last chat.turn).id}
```
...
let g1 = (.last chat.turn).id
"Germany's capital is Berlin." | .append chat.turn --meta {role: assistant continues: $g1}
"How big is Berlin?" | .append chat.turn --meta {role: user continues: (.last chat.turn).id}
```
