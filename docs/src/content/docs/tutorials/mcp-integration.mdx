...
In your monitoring window, you should see a `xs.service.mcp.create` and then a `xs.service.mcp.active` frame appear, indicating the service has started successfully.
...
r#'{
  run: {|frame, state|
    if ($frame.topic == "mcp.recv") {
      let content = .cas $frame.hash | from json | get -o result.content.0.text
...
}'# | .append xs.actor.meaning.detector.create
```
...
You could extend this pattern to create sophisticated automation - actors that trigger on specific responses, maintain counters, coordinate between multiple MCP servers, or trigger [actions](../../reference/actions/) based on detected events.
...
.append xs.service.mcp.term
```

In your monitoring window, you'll see the lifecycle frames: `xs.service.mcp.term`, `xs.service.mcp.fin.term`, and  as the service shuts down completely.
