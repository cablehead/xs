...
```nushell
'{"jsonrpc": "2.0", "id": 2, "method": "tools/call", "params": {"name": "echo", "arguments": {"message": "Hello from cross.stream!"}}}\n' | .append mcp.send
```
...
```nushell
'{"jsonrpc": "2.0", "id": 3, "method": "tools/call", "params": {"name": "add", "arguments": {"a": 100, "b": 200}}}\n' | .append mcp.send
```

The response (300) will appear in your monitoring window as a `mcp.recv` frame.
