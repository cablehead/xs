...
Topics ending with `.nu` register Nushell modules into the virtual filesystem (VFS). Any actor, service, or command script can then `use` these modules by their stable VFS path.
...
Each actor, service, or command sees the modules as they existed when it was registered. Modules appended later are only visible to components registered after them. Re-appending a module and then re-registering a component picks up the new version.
