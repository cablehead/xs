...
Commands emit events to track their execution. For a complete overview of all topic suffixes, see <Link to="/reference/topic-suffixes/">Topic Suffixes</Link>.
...
- `.last` – fetch the most recent frame for a topic in this context.
- `.cas` – read content from CAS by hash.
- `.get` – retrieve a frame by ID.
- `.remove` – delete a frame from the stream.

`cat` and `last` default to the command's context, while `append` accepts an
