...
Upon a successful start the handler appends a `<handler-name>.active` frame
with metadata:
...
For a complete overview of all topic suffixes, see <Link to="/reference/topic-suffixes/">Topic Suffixes</Link>.

```mermaid
stateDiagram-v2
    [*] --> Activating: .register event
    Activating --> Inactive: nushell parse error
...
`<handler-name>.inactive`. If unregistered due to an error, the frame
...
2. A frame is appended to `<handler-name>.inactive` with:
