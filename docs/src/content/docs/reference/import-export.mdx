...
Inside `xs eval` and handler scripts (actions, actors, services) a `.import`
builtin inserts a single frame verbatim, preserving its `id`. It is the
in-engine equivalent of the `xs import` CLI and the `POST /import` endpoint, and
the primitive the directory-level `.import <path>` helper is built on.
