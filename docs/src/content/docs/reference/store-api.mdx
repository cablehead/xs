...
Query parameters are the [read options](../read-options/): `follow`, `new`,
`after`, `from`, `limit`, `last`, `topic`, and `with-timestamp`. A `follow`
value may be `true` or a heartbeat interval in milliseconds.
...
Query parameters are the [read options](../read-options/) `last` (default: 1;
returns NDJSON when > 1), `follow`, and `with-timestamp`.
