...
| `--all`                         | Retrieve frames across all contexts             |
...
The eval command runs a Nushell script with access to store helper commands like `.cat`, `.append`, `.head`, `.get`, `.cas`, and `.remove`.
