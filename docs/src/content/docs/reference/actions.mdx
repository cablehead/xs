...
Actions share the unified lifecycle vocabulary documented in
<Link to="/reference/lifecycle/" />. The action subset is small:
...
Plus `xs.action.<name>.replaced` when a newer `.create` displaces an existing
definition. Actions don't emit `fin.ok`, `fin.error`, or `stopped` (no
long-lived task, no participation in `xs.stopping`).
