...
For the rationale see the [lifecycle design page](../../explanation/lifecycle-design/). For the
...
| `fin.ok` | out | Task ran to natural completion. For services, only under `restart: "never"`; the default policy restarts silently. |
...
[Module Topics](../topics/#module-topics) for the VFS mapping rules.
