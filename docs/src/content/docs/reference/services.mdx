...
- Auto-restart on natural completion (the pipeline drained without error). The
  ...
  crashes. Set `restart: "never"` to stop after one run instead.
...
| `restart` | string | `on-success` | What to do when the pipeline drains cleanly: `on-success` runs it again, `never` stops it (see Auto-restart) |
...
- `.append`, append a new frame. Metadata you provide is merged with a
  `service_id` identifying the service that wrote it.
...
internal restart, the service is still considered "running." This is the
default `restart: "on-success"` policy.

A pipeline that errors is not restarted under any policy. The service emits
`xs.service.<name>.fin.error` and stays down.

`restart: "never"` makes a clean completion terminal. The service emits
`xs.service.<name>.fin.ok` and stops, and it stays down across xs restarts like
any other `fin.*`. Use it when the pipeline ending is the point, such as a
shell the user exited, rather than a source that went away:

```nushell
r#'{
  restart: "never",
  run: {|| ^my-shell | lines }
}'# | .append xs.service.shell.create
```

To run it again, append a new `xs.service.<name>.create`.
...
- `xs.service.<name>.fin.ok`, the pipeline ran to completion under
  `restart: "never"`. Under the default policy a clean completion restarts
  and emits nothing.
