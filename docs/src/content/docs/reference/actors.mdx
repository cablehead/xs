...
r###'{
  ...
  # Optional: Only deliver matching topics to the actor.
  # Each entry is an exact topic, a "prefix.*" wildcard, or "*",
  # the same pattern language as read topic filters. A single
  # comma-separated string also works ("game.move.*,game.create").
  # The filter is applied at the read level, so non-matching frames
  # never reach the actor. The actor's own lifecycle frames
  # (xs.actor.<name>.create / .term) are always delivered, and
  # synthetic frames (xs.pulse, xs.threshold) bypass the filter,
  # so guard for them in the closure as usual. Omit to receive
  # every frame.
  topics: ["ping"]
...
| `topics`         | Topic patterns to process (list of strings, or one comma-separated string); applied as a read-level filter, so non-matching frames never reach the actor |
