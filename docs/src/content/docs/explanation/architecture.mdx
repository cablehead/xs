...
adds is durability against power loss or a kernel crash. Without it the part
of the journal not yet fsynced can be lost, and recovery then yields a stable
prefix of the stream, never a torn one.

The fsync policy decides when that fsync happens. `xs serve --fsync` takes
`always` (after every append), `interval:<ms>` (on a timer, only when
something was written), or `never`. The default is `interval:1000`. Embedders
pass the same choice to `Store::with_fsync`; `Store::flush` fsyncs on demand.
See the [cli reference](../../reference/cli/#fsync-policy).

### Expiry

A `time:` frame is written together with an entry in an expiry index, keyed
by the moment it expires. A background sweeper scans that index up to now on
a timer (`xs serve --ttl-sweep`, default one second) and removes every
expired frame it finds in one batch. Reads hide an expired frame the sweeper
has not reached yet, so the sweep interval only bounds how long the frame
stays on disk. See the [TTL reference](../../reference/ttl/).
...
lookups. Actor output uses this by default: the `out` record is stored as frame
metadata.
...
Actors can opt into CAS with `return_options: { target: "cas" }`.
