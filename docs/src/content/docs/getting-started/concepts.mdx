...
### Shared lifecycle, the `xs.<kind>` namespace

Services, actors, and actions all live under a runtime-owned namespace:

```
xs.service.<name>.<event>
xs.actor.<name>.<event>
xs.action.<name>.<event>
```

A topic that starts with `xs.` is managed by the runtime; everything else is
your data. You'll see `xs.service.log.create`, `xs.actor.echo.active`,
`xs.action.repeat.fin.term` and so on as you work with each kind.

All three share the same nine-event vocabulary: a user appends `create` to
start a thing, the runtime acks with `active` once it's running, the user
appends `term` to stop it, and the runtime emits a `fin.*` family of events
when it ends. The per-kind differences are small and surface mostly in what
the script *does* (a long-running pipeline for a service, an event-driven
closure for an actor, a stateless call for an action), not in how its
lifecycle is observed.

See the [lifecycle reference](../../reference/lifecycle/) for the full event vocabulary, the
...
| Error Handling   | Auto-restarts (with a 1-second delay) | Emits `fin.error`, stays down | Per-invocation               |
