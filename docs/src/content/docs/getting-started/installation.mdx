...
<TabItem label="nix">
This method helps avoid potential OpenSSL linking issues during compilation.

Create a `flake.nix` file:

```nix
{
  description = "Development environment for xs.";
  ...
  outputs = { self, nixpkgs }:
      ...
      in
      {
        devShell.x86_64-linux = pkgs.mkShell {
            buildInputs = [ pkgs.openssl pkgs.rustc pkgs.cargo ];
