...
<TabItem label="nix">
Run xs without installing:

```sh
nix run github:cablehead/xs -- --version
```
...
If you have the repository cloned, use the development shell with all Rust tooling and Nushell pre-configured:

```sh
nix develop
```

This starts an interactive Nushell session with `xs.nu` commands available. Press Ctrl+D or type `exit` to return to your previous shell.
...
```nix
{
  ...
  outputs = { self, nixpkgs, xs }: {
    # Use xs.packages.${system}.default in your configuration
  };
}
```
</TabItem>
